...
int main(int argc, char *argv[]){
   ...
   int ret, 
   ret_sel,fd_max, timeout = 0, bytestosend = 0;
   ...
   struct timespec t;
   const sigset_t pslect;
   char c;
   bool with_time = false;
   bool with_pslect = false;
   ...
   if (pid == 0){
      ...
      while (1){
         ...
         if (ret_sel < 0){
            ...
            fprintf(stderr, "Je suis fatigué d'attendre dans le fils bytesrecv (%ld) byteswrite(%ld)\n", bytesrecv, byteswrite);
            timeout++;
            if(timeout >= 3)
                break;
            sleep(10);
